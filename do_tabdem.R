...
# t1
ggplot(ctrldat, aes(t1, colour = vstat)) + geom_histogram(binwidth = 180) + 
  labs(x="Time between initial diagnosis and end of study / death (days)", y = "Frequency") +
  scale_colour_discrete(name = "Vital Status")

#t2
ggplot(ctrldat, aes(t2, colour = vstat)) + geom_histogram(binwidth = 30)+ 
  labs(x="Time between final HADS obs. and end of study / death (days)", y = "Frequency") + 
  scale_colour_discrete(name = "Vital Status")

#t3
ggplot(ctrldat, aes(t3, colour = vstat)) + geom_histogram(binwidth = 30)+ 
  labs(x="Time between penultimate and final HADS obs. (days)", y = "Frequency") +
  scale_colour_discrete(name = "Vital Status")

# Age
ggplot(ctrldat, aes(age, colour = vstat)) + geom_histogram(binwidth = 5)+ 
